...
try:
    from eventkit_geonode.settings import GEONODE_APPS
except:
...
def start_geoserver(options):
    ...
    from eventkit_geonode.settings import OGC_SERVER
...
def _reset():
    sh("rm -rf eventkit_geonode/development.db")
    sh("rm -rf eventkit_geonode/uploaded/*")
    _install_data_dir()
